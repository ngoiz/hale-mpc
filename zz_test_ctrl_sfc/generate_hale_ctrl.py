...
# for m = 16 only
input_vec = np.zeros((10, num_modes * 3 + 6))
x0 = np.zeros(10)
input_vec[5:, 2 * num_modes] = elevator * 5
input_vec[5:, 2 * num_modes + 1] = -elevator * 5
input_vec[5:, 2 * num_modes + 2] = rudder
...
def generate_aero_file():
    ...
    control_surface_chord[1] = m # m
    control_surface_hinge_coord[1] = -0.25 * 1 # nondimensional wrt elastic axis (+ towards the trailing edge)

    control_surface_type[2] = 0
    control_surface_deflection[2] = rudder_static_deflection
    control_surface_chord[2] = m // 2 #1
